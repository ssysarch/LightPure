...
def get_sigma_schedule( device, BETA_MIN, BETA_MAX):
    n_timestep = 4
    beta_min = BETA_MIN
    beta_max = BETA_MAX
    eps_small = 1e-3
...
class Diffusion_Coefficients():
    def __init__(self,device, BETA_MAX=0.2, BETA_MIN=0.05):
        self.sigmas, self.a_s, _ = get_sigma_schedule( device=device, BETA_MAX=BETA_MAX, BETA_MIN=BETA_MIN)
...
class Posterior_Coefficients():
    def __init__(self, device, BETA_MAX=0.2, BETA_MIN=0.05):
        _, _, self.betas = get_sigma_schedule( device=device, BETA_MAX=BETA_MAX, BETA_MIN=BETA_MIN)
