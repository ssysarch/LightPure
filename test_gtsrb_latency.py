...
def test(args):
    ...
    saved_model_classifier_18 = '/data2/diffpure_hossein/brandan/mobicom_gan/fine_tuned_ssim_15_classifier18_2/cifar10/experiment_cifar_default/netC_64.pth'
    ...
    saved_model_classifier_50 = '/data2/diffpure_hossein/brandan/mobicom_gan/fine_tuned_ssim_15/cifar10/experiment_cifar_default/netC_60.pth'
    ...
    with open(args.name, 'w') as file:
        ...
        for inputs, labels in data_loader:
            ...
            x_adv_gray = adversary_resnet_classifier1.run_standard_evaluation( inputs, labels)
            x_adv_black = x_adv_gray

            outputs_clean = robust_un(inputs )
            _, predicted_clean = torch.max(outputs_clean, 1)

            outputs_white = robust_un(x_adv_white )
            _, predicted_white = torch.max(outputs_white, 1)

            outputs_gray = robust_un(x_adv_gray)
            _, predicted_gray = torch.max(outputs_gray, 1)

            outputs_black = robust_un(x_adv_black)
            _, predicted_black = torch.max(outputs_black, 1)

            total += labels.size(0)

            correct_clean += (predicted_clean == labels).sum().item()
            correct_white += torch.logical_and(predicted_white == labels, predicted_clean== labels).sum().item()
            correct_gray += torch.logical_and(predicted_gray == labels, predicted_clean== labels).sum().item()
            correct_black += torch.logical_and(predicted_black == labels, predicted_clean== labels).sum().item()
